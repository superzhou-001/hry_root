...
@Component
public class OssRtcFileStrategy implements FileUploadStrategy {
    @Value("${rtc.oss.endpoint}")
    private String endpoint;
    /**
     * @Description: 云账号AccessKey有所有API访问权限，建议遵循阿里云安全最佳实践，创建并使用RAM子账号进行API访问或日常运维，请登录 https://ram.console.aliyun.com 创建。
     */
    @Value("${rtc.accessKeyId}")
    private String accessKeyId;
    @Value("${rtc.accessKeySecret}")
    private String accessKeySecret;
    @Value("${rtc.oss.privateBucketName}")
