...
public class ScmFileController extends BaseController {
   ...
   /* *//**
    ...
    public JsonResult remove (
            @ApiParam(name = "id", value = "文件id", required = true) @RequestParam Long id,
            @ApiParam(name = "infoId", value = "企业Id", required = true) @RequestParam Long infoId) {
        CuCustomer user = (CuCustomer) JWTContext.getUser(CuCustomer.class);
        JsonResult jsonResult = new JsonResult();
        ScmFile scmFile = scmFileService.get(id);
        if(!StringUtils.isEmpty(infoId) && infoId == user.getInfoId()){
            scmFile.setIsDelete(1);
            scmFileService.update(scmFile);
            return jsonResult.setSuccess(true);
        }else{
            return jsonResult.setSuccess(false).setMsg("删除失败");
        }
